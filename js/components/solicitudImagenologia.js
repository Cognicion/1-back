...
import {
  ...
  formatearHoraLocalDocumento,
  normalizarMedicoCatalogo,
  resolverMedicoCatalogo
} from "../services/solicitudImagenologiaPlantilla.js";

const escapar = (v) => String(v ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#039;" }[c]));
const normalizar = (v) => String(v ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
const hoy = () => new Date().toISOString().slice(0, 10);
const hora = () => new Date().toTimeString().slice(0, 5);
const idLocal = () => globalThis.crypto?.randomUUID?.() || `estudio-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;

function campo(paciente = {}, clave, alternos = []) {
  const fuentes = [paciente, paciente.datosInstitucionales || {}, paciente.signosVitales || {}, paciente.somatometria || {}];
  for (const fuente of fuentes) for (const nombre of [clave, ...alternos]) if (fuente[nombre] !== undefined && fuente[nombre] !== null && String(fuente[nombre]).trim()) return fuente[nombre];
  return "";
}
function fechaNacimiento(p) { return campo(p, "fechaNacimiento", ["fecha_nacimiento", "fechaDeNacimiento", "fechaNac"]); }
function edadDesde(fecha) { const n = new Date(`${fecha}T00:00:00`); if (!fecha || Number.isNaN(n.getTime())) return ""; const t = new Date(); let e = t.getFullYear() - n.getFullYear(); if (t.getMonth() < n.getMonth() || (t.getMonth() === n.getMonth() && t.getDate() < n.getDate())) e -= 1; return e >= 0 ? e : ""; }
function nombrePaciente(p) { return p.nombreCompleto || p.nombre || [p.nombres, p.apellidoPaterno, p.apellidoMaterno].filter(Boolean).join(" ") || ""; }
function perfilMedico(m = {}) { return { uid: m.uid || m.id || "", nombre: m.nombreCompleto || m.nombre || m.displayName || m.email || "", cargo: m.cargoCompleto || m.cargo || m.grado || m.especialidad || m.rol || "", cedulaProfesional: m.cedulaProfesional || m.cedula || "", servicio: m.servicio || m.servicioInstitucional || "", firmaId: m.firmaId || m.firma || "" }; }

function abrirVentana(html, titulo = "Solicitud de imagenología") {
  const ventana = window.open("", "_blank", "noopener,noreferrer");
  if (!ventana) throw Object.assign(new Error("La ventana de documento fue bloqueada"), { code: "document-popup-blocked" });
  ventana.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${escapar(titulo)}</title><style>body{margin:0;background:#fff;color:#111;font-family:Arial,sans-serif}.hoja{max-width:900px;margin:20px auto;padding:28px;box-sizing:border-box}@media print{.hoja{margin:0;max-width:none;padding:12mm}.no-print{display:none!important}}table{border-collapse:collapse;width:100%;font-size:12px}td,th{border:1px solid #444;padding:5px;vertical-align:top}.encabezado{text-align:center;border-bottom:2px solid #111;padding-bottom:8px}.firmas{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:45px;text-align:center}.linea{border-top:1px solid #111;padding-top:8px}.vacio{min-height:18px;border-bottom:1px solid #333}</style></head><body>${html}</body></html>`);
...
function htmlDocumento(datos) {
  ...
  const estudios = datos.estudios.map((e) => `<tr><td>${escapar(e.tipo)}</td><td>${escapar(e.modalidad)}</td><td>${escapar(e.nombre)}</td><td>${escapar(e.region)}</td><td>${escapar(e.contraste || "")}</td><td>${escapar(e.observaciones || "")}</td><td>${escapar(e.tipo === "Urgente" ? e.criterioUrgencia : "")}</td></tr>`).join("");
  return `<main class="hoja"><header class="encabezado"><strong>SECRETARÍA DE SALUD</strong><br>HOSPITAL PSIQUIÁTRICO "FRAY BERNARDINO ÁLVAREZ"<h2>${escapar(FORMATO_SOLICITUD_IMAGENOLOGIA.nombre)}</h2><strong>${escapar(FORMATO_SOLICITUD_IMAGENOLOGIA.clave)}</strong></header><h3>Datos de la solicitud</h3><table><tr><td>Fecha de solicitud: ${escapar(datos.solicitud.fecha)}</td><td>Hora: ${escapar(datos.solicitud.hora)}</td><td>Fecha de cita: ${escapar(datos.solicitud.fechaCita)}</td><td>Hora de cita: ${escapar(datos.solicitud.horaCita)}</td></tr></table><h3>Identificación del paciente</h3><table><tr><td>Nombre completo: ${escapar(p.nombreCompleto)}</td><td>Expediente: ${escapar(p.expediente)}</td></tr><tr><td>Fecha de nacimiento: ${escapar(p.fechaNacimiento)}</td><td>Edad: ${escapar(p.edad)}</td></tr><tr><td>CURP: ${escapar(p.curp)}</td><td>Sexo: ${escapar(p.sexo)} / Género: ${escapar(p.genero)}</td></tr></table><h3>Datos físicos y ubicación</h3><table><tr><td>PA: ${escapar(p.pa)}</td><td>Cama o consultorio: ${escapar(p.camaConsultorio)}</td><td>Peso: ${escapar(p.pesoKg)}</td><td>Talla: ${escapar(p.tallaM)}</td></tr><tr><td colspan="2">Servicio solicitante: ${escapar(p.servicio)}</td><td colspan="2">Alergias: ${escapar(p.alergias)}</td></tr></table><h3>Datos clínicos y sospecha diagnóstica</h3><div class="vacio">${escapar(datos.datosClinicos).replace(/\n/g, "<br>")}</div><h3>Estudios solicitados</h3><table><thead><tr><th>Tipo</th><th>Modalidad</th><th>Estudio</th><th>Región</th><th>Contraste</th><th>Observaciones técnicas</th><th>Criterio de urgencia</th></tr></thead><tbody>${estudios}</tbody></table><h3>Datos médicos</h3><table><tr><td>Médico solicitante: ${escapar(datos.medicoSolicitante.nombre)}<br>${escapar(datos.medicoSolicitante.cargo)}<br>Cédula: ${escapar(datos.medicoSolicitante.cedulaProfesional)}<br>Servicio: ${escapar(datos.medicoSolicitante.servicio)}</td><td>Médico adscrito: ${escapar(datos.medicoAdscrito.nombre)}<br>Cédula de especialidad: ${escapar(datos.medicoAdscrito.cedulaEspecialidad)}</td></tr><tr><td>Firma:</td><td>Firma:</td></tr><tr><td>Recibe la solicitud:</td><td></td></tr></table><footer style="margin-top:24px;font-size:11px;text-align:center">Solicitud institucional · Snapshot de datos al momento de emisión · No equivale a resultado del estudio</footer></main>`;
}

function datosWord(datos) {
...
export function abrirSolicitudImagenologia({ paciente = {}, medico = {}, uidPaciente = "", onPersist, onAudit, servicio = "", catalogoMedicos = [] } = {}) {
  ...
  const p = { expediente: resolverExpedienteInstitucional(paciente), nombreCompleto: nombrePaciente(paciente), fechaNacimiento: fechaNacimiento(paciente), curp: campo(paciente, "curp", ["CURP"]), sexo: campo(paciente, "sexo"), genero: campo(paciente, "genero", ["identidadGenero"]), pa: campo(paciente, "pa", ["presionArterial"]), camaConsultorio: campo(paciente, "cama", ["consultorio", "camaConsultorio"]), pesoKg: campo(paciente, "peso"), tallaM: campo(paciente, "talla"), servicio: servicio || campo(paciente, "servicioInstitucional", ["servicio"]) || medicoInicial.servicio, alergias: campo(paciente, "alergias") };
  ...
  ].filter((item) => item?.id || item?.uid).map((item) => {
    const medicoNormalizado = normalizarMedicoCatalogo(item);
    return [medicoNormalizado.id, medicoNormalizado];
  })).values());
  const opcionesMedicos = (seleccionado = "") => `<option value="">Seleccionar médico</option>${catalogo.map((item) => `<option value="${escapar(item.id)}" ${item.id === seleccionado ? "selected" : ""}>${escapar(item.nombre || "Sin nombre")}</option>`).join("")}<option value="__otro__" ${seleccionado === "__otro__" ? "selected" : ""}>Otro…</option>`;
  ...
  const estudioInput = form.elements.buscarEstudio;
  if (estudioInput) {
    const selectorEstudio = document.createElement("select");
    selectorEstudio.name = "estudioCatalogo";
    selectorEstudio.setAttribute("aria-label", "Estudio del catálogo");
    selectorEstudio.innerHTML = `<option value="">Seleccionar estudio del catálogo</option>${CATALOGO_ESTUDIOS_IMAGEN.filter((item) => item.activo && item.id !== "otro-estudio-imagen").map((item) => `<option value="${escapar(item.id)}">${escapar(item.nombre)}</option>`).join("")}<option value="otro-estudio-imagen">Otro</option>`;
    estudioInput.closest("label")?.insertBefore(selectorEstudio, estudioInput);
    selectorEstudio.addEventListener("change", () => {
      const estudio = CATALOGO_ESTUDIOS_IMAGEN.find((item) => item.id === selectorEstudio.value);
      estudioInput.value = estudio?.id === "otro-estudio-imagen" ? "" : (estudio?.nombre || "");
      if (estudio?.region) form.elements.region.value = estudio.region;
      if (estudio?.modalidad) form.elements.modalidad.value = estudio.modalidad;
      estudioInput.placeholder = estudio?.id === "otro-estudio-imagen" ? "Escribe manualmente el estudio solicitado" : "Seleccionar estudio o elegir Otro";
    });
  }
  ...
  function agregarEstudio() { const seleccionado = CATALOGO_ESTUDIOS_IMAGEN.find((e) => e.id === valor("estudioCatalogo")); const esOtro = seleccionado?.id === "otro-estudio-imagen"; const nombreManual = valor("buscarEstudio").trim(); const nombre = esOtro ? nombreManual : seleccionado?.nombre || nombreManual; if (!nombre) return; const item = { id: idLocal(), catalogoId: seleccionado?.id || "manual", tipo: valor("tipo") || "Ordinario", modalidad: esOtro ? "otro" : valor("modalidad"), nombre, region: valor("region") || seleccionado?.region || "", protocolo: valor("protocolo"), contraste: valor("contraste"), observaciones: valor("observacionesEstudio"), criterioUrgencia: valor("criterioUrgencia") }; const duplicado = estado.estudios.some((e) => normalizar(e.nombre) === normalizar(item.nombre) && e.modalidad === item.modalidad); if (duplicado && !window.confirm("Este estudio ya está agregado. ¿Deseas añadirlo nuevamente?")) return; estado.estudios.push(item); renderizarEstudios(); actualizarFaltantes(); }
  ...
  function valoresPlantilla(actual) {
    ...
    return {
      ...
      MEDICO_SOLICITANTE: solicitante.nombre && !String(solicitante.nombre).trim().endsWith(".") ? `${solicitante.nombre}.` : solicitante.nombre,
  ...
  async function descargarWord() {
    ...
    actual.paciente.expediente = p.expediente;
    actual.solicitud.fecha = `${ahora.getFullYear()}-${String(ahora.getMonth() + 1).padStart(2, "0")}-${String(ahora.getDate()).padStart(2, "0")}`;
    actual.solicitud.hora = formatearHoraLocalDocumento(ahora);
    const faltan = faltantes(actual);
    console.debug("[FTO-IMG] expediente institucional:", actual.paciente.expediente);
    console.debug("[FTO-IMG] médico solicitante:", actual.medicoSolicitante);
    console.debug("[FTO-IMG] médico adscrito:", actual.medicoAdscrito);
    console.debug("[FTO-IMG] fecha descarga:", formatearFechaDocumento(actual.solicitud.fecha));
    console.debug("[FTO-IMG] hora descarga:", actual.solicitud.hora);
  ...
  renderizarEstudios(); actualizarFaltantes();
  return { cerrar: () => overlay.remove(), datos };
}
