...
export function createBiocellularBackground(host, incomingOptions = {}) {
  ...
  const resize = () => { const rect = host.getBoundingClientRect(); width = Math.max(1, rect.width); height = Math.max(1, rect.height); const dpr = profile.dpr; canvas.width = Math.round(width * dpr); canvas.height = Math.round(height * dpr); canvas.style.width = `${width}px`; canvas.style.height = `${height}px`; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); };
  const draw = (time) => {
    if (disposed) return;
  ...
  const visibility = () => { paused = Boolean(options.pauseHidden && document.visibilityState !== "visible"); if (!paused && !raf) raf = requestAnimationFrame(draw); };
  const observer = typeof ResizeObserver === "function" ? new ResizeObserver(resize) : null;
  if (observer) observer.observe(host); else window.addEventListener("resize", resize, { passive: true });
  document.addEventListener("visibilitychange", visibility, { passive: true }); resize(); raf = requestAnimationFrame(draw);
