...
export function buildImportedNotePayload({ document, confirmedType, sourceFile, importId, user }) {
  ...
  return {
    ...
    plan: [sectionValue(sections, "plan"), sectionValue(sections, "tratamiento"), sectionValue(sections, "medicamentos")].filter(Boolean).join("\n\n"),
    tratamiento: [sectionValue(sections, "tratamiento"), sectionValue(sections, "medicamentos")].filter(Boolean).join("\n\n") || sectionValue(sections, "plan"),
    fechaNotaInput: date,
    horaNotaInput: hour,
