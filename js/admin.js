...
function renderizarAccesoMotorPatrones() {
  ...
  seccion.innerHTML = `
    <div class="tabla-header"><div><h2>Explorador de patrones léxicos</h2><p>Busca patrones frecuentes no redundantes entre pacientes. Es una herramienta de exploración documental, no una conclusión clínica.</p></div><div class="patrones-acciones"><button id="btnAnalizarPatronesTexto" type="button">Analizar textos</button><button id="btnReintentarPatronesTexto" type="button" hidden>Reintentar lectura</button><button id="btnExportarPatronesExcel" type="button" disabled>Exportar Excel</button><button id="btnExportarPatronesCsv" type="button" disabled>Exportar CSV</button><button id="btnLimpiarPatronesTexto" type="button">Limpiar resultados temporales</button></div></div>
    ...
    <div class="tabla-scroll"><table><thead><tr><th>Patrón desidentificado</th><th>Forma normalizada</th><th>Firma léxica</th><th>Utilidad</th><th>Frecuencia total</th><th>Notas</th><th>Pacientes</th><th>Médicos</th><th>Primera aparición</th><th>Última aparición</th><th>Palabras</th></tr></thead><tbody id="tablaPatronesTexto"><tr><td colspan="11">Sin resultados temporales.</td></tr></tbody></table></div>`;
...
async function renderizarAccesoConocimientoSofia() {
  ...
  if (!nav || !main) return;
  try {
    const modulo = await import("./admin/clinicalKnowledge/clinicalKnowledgeController.js?v=20260831-sofia-admin-density-v1");
...
window.completarPerfilPendienteAdmin = async function(uidUsuario) {
  const usuario = usuariosAdmin.find((item) => item.id === uidUsuario);
  ...
  if (![MEMBERSHIP_TYPES.FREE, MEMBERSHIP_TYPES.PRO].includes(tipoMembresia)) {
    alert("Selecciona una membresía válida.");
    return;
  }

  const cuenta = usuario.email || uidUsuario;
