...
const utilidad = (fila) => {
  const tier = ({ high: "Alta", moderate: "Moderada", exploratory: "Exploratoria", low: "Baja" })[fila.utilityTier] || "Exploratoria";
  const score = Number.isFinite(Number(fila.relevanceScore)) ? `${Math.round(Number(fila.relevanceScore) * 100)}/100` : "—";
  return `${tier} · ${score}`;
};
