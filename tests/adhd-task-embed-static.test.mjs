...
test("el modo embebido se declara antes del primer render en todas las tareas legacy", () => {
  assert.match(themePreload, /get\("adhd"\) === "1"[\s\S]*?get\("embed"\) === "1"[\s\S]*?window\.self !== window\.top/u);
  assert.match(themePreload, /root\.dataset\.cognicionEmbed = "adhd-task"/u);
  assert.match(themePreload, /adhd-task-embed\.css\?v=20260902-adhd-task-embed-v1/u);
  ...
  for (const page of LEGACY_TASK_PAGES) {
    ...
    assert.match(pageSource, /theme-preload\.js\?v=20260902-adhd-task-embed-v1/u, `${page} debe invalidar el bootstrap síncrono anterior`);
    assert.match(pageSource, /themeBootstrap\.js\?v=20260902-adhd-task-embed-v1/u, `${page} debe invalidar el bootstrap modular anterior`);
    assert.match(pageSource, /reportes\.js\?v=20260902-adhd-task-embed-v1/u, `${page} debe invalidar el widget de reportes anterior`);
    assert.match(pageSource, /accesosRapidos\.js\?v=20260902-adhd-task-embed-v1/u, `${page} debe invalidar los accesos rápidos anteriores`);
  }
});
...
test("navbar, accesos, reportes y fondo global se omiten dentro del iframe", () => {
  ...
  assert.match(quickAccess, /tareaAdhdEmbebida[\s\S]*?return tareaAdhdEmbebida/u);
...
test("el host oculto no roba la fila central y el iframe conserva toda el área útil", () => {
  assert.match(programHtml, /rehabilitacion-tdah\.css\?v=20260902-adhd-task-embed-v2/u);
  assert.match(programHtml, /rehabilitacion-tdah\.js\?v=20260902-adhd-task-embed-v1/u);
...
test("CPT embebido abre instrucciones de práctica y usa un viewport compacto", () => {
  assert.match(cptHtml, /cpt-adhd-embed\.css\?v=20260902-adhd-task-embed-v1/u);
  assert.match(cptHtml, /cpt\.js\?v=20260902-adhd-task-embed-v1/u);
  assert.match(cptController, /const embeddedTaskMode = adhdTaskMode[\s\S]*?get\("embed"\) === "1"/u);
...
test("Go/No-Go embebido inicializa aunque DOMContentLoaded ya haya ocurrido", () => {
  assert.match(goNoGoHtml, /go-nogo-adhd-embed\.css\?v=20260902-adhd-task-embed-v1/u);
  assert.match(goNoGoHtml, /go-nogo\.js\?v=20260902-adhd-task-embed-v1/u);
  assert.match(goNoGoController, /const embeddedTaskMode = adhdTaskMode[\s\S]*?get\("embed"\) === "1"/u);
...
test("Stroop y N-Back embebidos mantienen práctica, respuestas y resultados dentro del iframe", () => {
  assert.match(stroopHtml, /stroop-adhd-embed\.css\?v=20260902-adhd-task-embed-v1/u);
  ...
  assert.match(nbackHtml, /nback-adhd-embed\.css\?v=20260902-adhd-task-embed-v1/u);
...
test("Escucha dicótica conserva la preparación auditiva real en un layout compacto", () => {
  assert.match(dichoticHtml, /escucha-dicotica-adhd-embed\.css\?v=20260902-adhd-task-embed-v1/u);
  assert.match(dichoticHtml, /btnValidarAudiosEmbed[\s\S]*?btnPracticaEmbed/u);
  assert.match(dichoticHtml, /escucha-dicotica\.js\?v=20260902-adhd-task-embed-v1/u);
  assert.match(dichoticController, /const embeddedAdhdTaskMode = adhdTaskMode[\s\S]*?get\("embed"\) === "1"/u);
